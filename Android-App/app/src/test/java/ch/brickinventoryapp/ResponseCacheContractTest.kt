...
/**
 * Autofokus der Kamera-Ansichten.
 *
 * Wiederkehrende Regression: Der Scanner stellt nicht scharf. Zwei Ursachen
 * traten bisher auf —
 *
 *   1. Ein periodisch aufgerufenes startFocusAndMetering („Fokus-Pump") zwang
 *      die Kamera immer wieder in eine neue Messung, der Fokus wanderte
 *      dauernd. Im Code steht dazu ein „Nicht wieder einbauen".
 *   2. CONTROL_AF_MODE nur am Preview-Use-Case. CameraX führt die
 *      Konfigurationen aller gebundenen Use Cases zu EINEM Repeating-Request
 *      zusammen; steht der Modus nur an einem, entscheidet je nach Gerät die
 *      andere Konfiguration mit.
 *
 * Dieser Test hält beide Punkte fest, damit die Regression nicht ein drittes
 * Mal auftritt. Er braucht kein Android — er liest die Quelldateien.
 */
class CameraFocusConfigTest {

    private val files = listOf(
        "src/main/java/ch/brickinventoryapp/ui/screens/BarcodeScannerScreen.kt",
        "src/main/java/ch/brickinventoryapp/ui/screens/SetupScreen.kt",
    )

    private fun read(rel: String): String {
        // Test läuft im app/-Modulverzeichnis
        val f = java.io.File(rel)
        assert(f.exists()) { "Datei nicht gefunden: $rel" }
        return f.readText()
    }

    /** Kommentare entfernen — der Warnblock oben in den Dateien nennt die
     *  Konstante selbst und würde jede Zählung verfälschen. */
    private fun stripComments(src: String) = src
        .replace(Regex("/\\*[\\s\\S]*?\\*/"), "")
        .replace(Regex("//[^\n]*"), "")

    @Test
    fun `beide Use Cases setzen den kontinuierlichen Autofokus`() {
        for (rel in files) {
            val code = stripComments(read(rel))
            val count = Regex("CONTROL_AF_MODE_CONTINUOUS_PICTURE").findAll(code).count()
            assertEquals(
                "In $rel muss der AF-Modus an Preview UND ImageAnalysis stehen — " +
                "CameraX führt beide Use Cases zu einem Repeating-Request zusammen. " +
                "NICHT diesen Test anpassen, sondern die fehlende Stelle ergänzen.",
                2, count
            )
        }
    }
    ...
    @Test
    fun `kein periodischer Fokus-Pump`() {
        for (rel in files) {
            val code = stripComments(read(rel))
            // startFocusAndMetering ist NUR für Tap-to-Focus erlaubt — also
            // innerhalb eines Touch-Listeners, nie in einer Schleife oder einem
            // Timer.
            for (m in Regex("(?s).{240}startFocusAndMetering").findAll(code)) {
                val before = m.value
                assert(!before.contains("delay(") && !before.contains("Timer") &&
                       !before.contains("scheduleAtFixedRate") && !before.contains("while (")) {
                    "$rel: startFocusAndMetering scheint periodisch aufgerufen zu werden — " +
                    "das lässt den Fokus wandern"
                }
            }
        }
    }
