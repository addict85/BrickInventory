/**
 * Welchen Zustand holt der Preislauf für einen Wunsch?
 *
 * ── Woher die Frage kommt ───────────────────────────────────────────────────
 * Marco: „Die Preise für die Wunschliste sollen auch hier benutzerabhängig
 ...
 * sets-Zeile — beides hat ein Wunsch nicht. Er fiel damit auf
 * DEFAULT_PRICE_CONDITION durch, und das ist 'U' (utils/finance/preise.ts).
 * Gemessen hiess das: Jeder Wunsch wurde als GEBRAUCHT geholt, auch der auf
 * ein neues Set — und der Verlauf im Wunsch-Detail füllte sich für den
 * falschen Zustand.
 *
 * ── Warum gegen eine echte Datenbank ────────────────────────────────────────
 * Die Regel liest aus DREI Tabellen mit einer Vorrangregel zwischen ihnen
 * (set_acquisitions → sets → wishlist). Genau das Zusammenspiel ist die
 * Aussage; mit erfundenen Zeilen wäre davon nichts geprüft.
 *
 * ── Gegenproben (durchgeführt, Ergebnis im Commit) ──────────────────────────
 *   a) Die wishlist-Abfrage in zustaendeJeSet() entfernt → rot in Zeile 68,
 *      „der Wunsch desselben Sets bei einem anderen Nutzer als gebraucht".
 *      NICHT in der Zeile davor: Ohne Wunschliste fällt die Funktion hier auf
 *      'N' zurück, und der erste Wunsch ist zufällig auch 'N'. Deshalb steht
 *      derselbe Wunsch zweimal da, einmal je Zustand — die Aussage
 *      „benutzerabhängig" ist erst mit beiden Nutzern geprüft.
 *   b) `for (const c of gewuenscht...)` nur im else-Zweig → „Bestand und
 *      Wunsch in verschiedenen Zuständen ergeben beide" wird rot.
...
test('Preislauf: der Zustand eines Wunsches ist der des Nutzers', { concurrency: 1 }, async (t) => {
  ...
  try {
    // ── Zwei Nutzer, DASSELBE Set, verschiedene Wünsche ─────────────────────
    ...
    await db.run(`INSERT INTO wishlist (user_id, set_number, condition) VALUES ($1,'10305-1','N')`, [a]);
    await db.run(`INSERT INTO wishlist (user_id, set_number, condition) VALUES ($1,'10305-1','U')`, [b]);

    assert.deepEqual((await zustaendeJeSet(a, ['10305-1'])).get('10305-1'), ['N'],
      'ein Wunsch auf ein NEUES Set muss als neu geholt werden — vorher ergab die ' +
      'Vorgabe DEFAULT_PRICE_CONDITION hier still ein "U"');
    assert.deepEqual((await zustaendeJeSet(b, ['10305-1'])).get('10305-1'), ['U'],
      'und der Wunsch desselben Sets bei einem anderen Nutzer als gebraucht');

    // ── Beide Zustände gewünscht: beide Preise werden gebraucht ─────────────
    await db.run(`INSERT INTO wishlist (user_id, set_number, condition) VALUES ($1,'21318-1','N'),($1,'21318-1','U')`, [a]);
    assert.deepEqual(sortiert((await zustaendeJeSet(a, ['21318-1'])).get('21318-1')), ['N','U'],
      'neu und gebraucht sind zwei Wünsche — für beide braucht das Detail einen Preis');

    // ── Bestand ohne Wunsch: unverändert ────────────────────────────────────
    ...
    // ── Bestand UND Wunsch im anderen Zustand: beide ────────────────────────
    ...
    await db.run(`INSERT INTO wishlist (user_id, set_number, condition) VALUES ($1,'42100-1','U')`, [a]);
    assert.deepEqual(sortiert((await zustaendeJeSet(a, ['42100-1'])).get('42100-1')), ['N','U'],
      'Bestand und Wunsch in verschiedenen Zuständen ergeben beide Abrufe');
    ...
    // Wunschliste. Diese Zusicherung hält sie zusammen.
    ...
    for (const uid of [a, b]) {
      await db.run(`DELETE FROM wishlist WHERE user_id=$1`, [uid]).catch(() => {});
