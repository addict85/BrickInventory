...
function _key(req, username) {
  return `login|${_ipOf(req)}|${(username || '').toLowerCase()}`;
}
 ...
 */
async function _read(key, windowMin) {
  try {
 ...
 */
async function _bump(key, windowMin) {
  try {
    await db.run(
 ...
 */
async function checkLoginAllowed(req, username) {
...
async function recordLoginSuccess(req, username) {
 ...
 */
function ipThrottle(bucket, max, windowMs) {
